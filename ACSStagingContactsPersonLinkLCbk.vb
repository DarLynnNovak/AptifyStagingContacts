...
Public Class ACSStagingContactsPersonLinkLCbk
    ...
    Protected Overrides Sub OnFormTemplateLoaded(ByVal e As FormTemplateLoadedEventArgs)
        Try
            ...
            'End If
            partnerPersonId = Me.FormTemplateContext.GE.GetValue("id")
            ...
            PersonLookup(txtSearchFirstName.Value, txtSearchLastName.Value, "")
    ...
    Private Sub btnCreate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        Try
            ...
            sPersonFirstName = FormTemplateContext.GE.GetValue("FirstName")
            sPersonLastName = FormTemplateContext.GE.GetValue("LastName")
            ...
            Select Case MsgBox("Are you sure you want to create " & sPersonFirstName & " " & sPersonLastName & " ?", MsgBoxStyle.YesNo, "Create Person")
                Case MsgBoxResult.Yes
                    ...
                    Dim oACSStagingContactsId As Long = FormTemplateContext.GE.RecordID

                    ACSStagingContacts = CType(m_oAppObj.GetEntityObject("ACSStagingContacts", oACSStagingContactsId), AptifyGenericEntity)
                    Dim acsacadrecordid As Long = Me.FormTemplateContext.GE.GetValue("RecordID")

                    If CLng(lbEntityId.Value) = acsacadNominationEntityId Then


                        Dim ACSAcadNomination As AptifyGenericEntity
                        ACSAcadNomination = CType(m_oAppObj.GetEntityObject("ACSAcadNomination", acsacadrecordid), AptifyGenericEntity)

                        With ACSAcadNomination
                            .SetValue("NomineePersonId", oPerson.RecordID)
                        End With
                    ...
                    If CLng(lbEntityId.Value) = acsJPPAppEntityId Then
                        ...
                        ACSJPPApp = CType(m_oAppObj.GetEntityObject("ACSJPPApplication", acsacadrecordid), AptifyGenericEntity)

                        With ACSJPPApp
                            .SetValue("ApplicantId", oPerson.RecordID)
                        End With
                        ACSJPPApp.Save()
            ...
            If lGridID > 0 Then
                ...
                Select Case MsgBox("Are you sure you want to update " & oPerson.GetValue("FirstName").ToString & " " & oPerson.GetValue("LastName").ToString & " ?", MsgBoxStyle.YesNo, "Person Update")
                    Case MsgBoxResult.Yes
                        ...
                        If sErr.Length > 0 Then
                            ShowMessage("There was a problem updating the person record " & sPersonFirstName & " " & sPersonLastName & ". Please refer to the Aptify exception log.", True)
                        Else
                            lblSearch.Text = "Person " & sPersonFirstName & " " & sPersonLastName & " updated successfully!"
                        End If
                        Dim ACSStagingContacts As AptifyGenericEntity
